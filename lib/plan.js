...
const OBJECTIVE_SHARES = {
  lift_tickets:  { ctv: 0.40, display: 0.35, audio: 0.25 },
  season_passes: { ctv: 0.42, display: 0.33, audio: 0.25 },
  lodging:       { ctv: 0.45, display: 0.35, audio: 0.20 },
  local_visits:  { ctv: 0.32, display: 0.40, audio: 0.28 }
};

const OBJECTIVE_WHY = {
  lift_tickets: "Lift tickets are urgent and weather-sensitive. CTV builds awareness, display carries frequency and response, audio delivers immediate localized urgency.",
  season_passes: "Season passes need broad awareness, advance consideration, and repeated visual storytelling, so CTV carries more weight.",
  ...
  local_visits: "Local visits and tubing depend on short drive times and weekend decisions, so display and audio are weighted more heavily."
};
...
const TYPE_TILT = {
  destination:        { ctv:  0.06, display: -0.03, audio: -0.03, why: "A destination resort is selling a trip, not an afternoon, so more weight goes to the big screen." },
  regional_overnight: { ctv:  0.00, display:  0.00, audio:  0.00, why: "A regional resort with lodging sits in the middle of the mix." },
  day_drive:          { ctv: -0.06, display:  0.03, audio:  0.03, why: "A day-drive resort competes on immediacy, so frequency and in-car audio pull ahead of the big screen." },
  tubing_focus:       { ctv: -0.08, display:  0.04, audio:  0.04, why: "A tubing and beginner hill sells a weekend decision to a local family, so response channels lead." }
};

function budgetPlan(body) {
  const budget = clamp(num(body.monthly_budget, 6000), 2500, 100000);
  const objective = String(body.campaign_objective || "lift_tickets");
  const type = String(body.resort_type || "regional_overnight");

  const base = OBJECTIVE_SHARES[objective] || OBJECTIVE_SHARES.lift_tickets;
  const tilt = TYPE_TILT[type] || TYPE_TILT.regional_overnight;

  let shares = {
    ctv: clamp(base.ctv + tilt.ctv, 0.15, 0.60),
    display: clamp(base.display + tilt.display, 0.15, 0.60),
    audio: clamp(base.audio + tilt.audio, 0.15, 0.60)
  };
  const total = shares.ctv + shares.display + shares.audio;
  shares = {
    ctv: shares.ctv / total,
    display: shares.display / total,
    audio: shares.audio / total
  };

  const allocation = {
    ctv: round(budget * shares.ctv),
    display: round(budget * shares.display),
    audio: round(budget * shares.audio)
  };
  const impressions = {
  ...
  if (current) {
    ...
    if (!mentions(/audio|spotify|podcast|pandora|radio/)) {
      gaps.push("No digital audio appears in the current media. It is the channel that reaches drive-market guests while they are actually in the car.");
    }
  }
  ...
  return {
    ...
    objective,
    resort_type: type,
    objective_rationale: OBJECTIVE_WHY[objective] || OBJECTIVE_WHY.lift_tickets,
    resort_type_rationale: tilt.why,
    allocation,
    impressions,
    channels: [
      {
      ...
      {
        key: "display", channel: "Programmatic Display", budget: allocation.display,
        cpm_assumption: CPMS.display, impressions: impressions.display,
        share_percent: round(shares.display * 100),
        role: "Carries high-frequency reminders: snow-condition messages, lift-ticket and lodging offers, retargeting, and date-sensitive promotions across weather apps and news sites. Lowest planning CPM, so it delivers the most impressions."
      },
      {
        key: "audio", channel: "Digital Audio", budget: allocation.audio,
        cpm_assumption: CPMS.audio, impressions: impressions.audio,
        share_percent: round(shares.audio * 100),
        role: "Reaches commuters, travelers, and streaming and podcast audiences when snow or cold makes a trip feel relevant. Companion banners add a clickable response path."
      }
    ],
...
function targetLocations(body, audience) {
  ...
  return {
    ...
    why_it_matters:
      "A generic regional campaign reaches a large number of people with no interest in skiing. Visitation-based audiences focus media on people whose physical behavior suggests winter-sports interest — devices seen at a competing mountain, a ski retailer, a winter sports event, or a club location.",
    lookback_recommendation:
      "Build seasonal visitor audiences from prior resort visitors where platform and privacy rules permit, then suppress current passholders when acquisition is the objective."
  };
}
...
const WEEK_PLAYS = {
  "Aggressive activation": [
    ...
    "Run urgent digital audio messaging",
